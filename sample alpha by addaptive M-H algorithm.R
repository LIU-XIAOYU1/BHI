#################################################
###### sample alpha by addaptive M-H algorithm###
#################################################
...
cof_range=2
alpha_0=c(0,0,0)                 #prior mean of alpha
sigma.alpha=c(100,100,100)           #prior variance of alpha
prop.s= c(0.1,0.1,0.1)                   #proposal setting of alpha in t distribution
sigma.beta=10
a_lam=1
b_lam=1
...
ind_fun<-function(x,beta,xx,sigma.beta,te1,te2,j)
 ...
 alpha_fun<-function(X,u,alpha.new,alpha,alpha_0,sigma.alpha,prop.s)
 {
   Z<-cbind(1,X)
   P<-ncol(Z)
   ...
   lpold<-sum(dnorm(alpha,alpha_0,sigma.alpha,log = TRUE))
   lpnew<-sum(dnorm(alpha.new,alpha_0,sigma.alpha,log = TRUE))
   
   ratio<-lpnew+lnew-(lpold+lold)
   return(ratio)
 }
 ...
 BHIsur<-function(xcov,Z,L,R,status,cof_range=2,niter=10000,burn=niter/4)
 {
   ...
   for (iter in 1:niter)
   {
     ...
     alpha<-alpha.track[iter,]
     
     for (j in 1:p+1)
     {
       alpha.new<-alpha
       alpha.new[j]<-rnorm(1,alpha[j],prop.s[j])
       U<-runif(1)
       ratio<-alpha_fun(X=X,u=u.track[iter,],alpha.new=alpha.new,alpha=alpha,alpha_0=alpha_0,sigma.alpha=sigma.alpha,prop.s=prop.s)
       if(log(U)<ratio)
       {
         alpha<-alpha.new
         accept[j]<-accept[j]+1
   ...
   list(alpha=f.alpha,beta=f.beta,gamma=f.gamma,accept=accept/niter,a_lam=a_lam,b_lam=b_lam)  
 ...
 write.csv(mydata,file = paste("sample alpha by addaptive M-H algorithm ",".csv",sep=""))
