...
accept.rep=matrix(0,nrow = repl,ncol=1)
...
plot(timepoint,-log(1-pweibull(timepoint,shape = 6,scale=8)),type = "l",xlab = "T",ylab = "Lambda[0](T)",lwd=3, col="black")
for(k in 1:q)
{
...
plot(timepoint,1-pweibull(timepoint,shape = 6,scale = 8),type="l",col="black",lwd=3,xlab="T", 
     ylab=expression(S(T)))
q<-nrow(gamma.rep)
for(l in 1:q)  
{
...
mydata<-data.frame(alpha.rep,beta.rep,gamma.rep)
# write.csv(mydata,file = paste("Rep ",repl,".csv",sep=""))
